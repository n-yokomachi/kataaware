...
## 部屋の作り直し（オーナーの指示による 2 周目）

家具を置いた後、オーナーの指示で部屋の作りそのものを直した。記録として残す。

- 壁・天井・床を打ちっぱなしのコンクリートにした。絵は PIL で作り、`Assets/Textures/Concrete.png`（型枠の目地とセパレータの穴）と `ConcreteFloor.png`（打ち継ぎの目地のみ）の 2 枚。継ぎ目が出ないよう 3x3 に並べてぼかしてから中央を切り出している。1 枚 = 1.8 メートル四方なので、6x3 の壁は (3.333, 1.667)、6x6 の床と天井は (3.333, 3.333) で貼る
- 窓のせり出しを直した。kit の `wallWindow.glb` は壁の板そのもので、部屋の中へ 0.129 メートル出てしまう。壁を 4 枚（左・右・腰・まぐさ）に割って穴を空け、枠・方立・無目・ガラスを箱で組んで穴の中に収めた。両方の窓を同じ作りにしてある
- モニターを 1 枚から 5 枚にした。アームの支柱と桁に、下段 3 枚・上段 2 枚を吊る。すべて椅子の方を向くよう角度を変え、上段は 10 度下げてある。枠は 0.86 x 0.53 メートル。鍵盤と鼠も机に載せた
- 寝台を革張りの長椅子に替えた（`loungeSofa.glb`、局所尺度 2.7/2.1/2.2 で 2.64 メートル）。枕と崩した掛け布を座面に置き、ここで寝起きしていることが分かるようにした
- 本棚を隅へ、鉢を机の脇へ移し、冷蔵庫を `kitchenFridgeLarge` に替えて、冷蔵庫と長椅子のあいだに 1.1 メートルの空きを取った
- 天井の照明に器具を付けた（`lampSquareCeiling.glb`）。笠は電球色にして、窓の紫と見分けが付くようにしている
- メモリ架を脇机へ、灰皿と煙草を机の左手（メモリ架のあった所）へ移した。卓上のラジオは外した
- 材質を暗い側へ寄せた。`Concrete` / `ConcreteFloor` / `ConcreteCeiling` / `Leather` / `Ink` / `ClothDark` / `LampGlow` / `NightGlass` を新設
- 場面が参照していない `.glb` は消した。`LICENSES.md` の表も今の状態に書き直してある

見た目の判断でつまずいた点。小さく縮めた画で色を見ると対比で誤る。長椅子の肘掛けが白く光って見えたので材質を疑ったが、拡大して画素を読むと RGB 58 前後の暗い面だった。**怪しいと思ったら、まず画素を読む**。

### 仕上げに直したこと

- もう一方の窓（寝台側の壁）にもカーテンを付けた。机側と同じ寸法で、軸だけ入れ替えてある
- 革が単色で樹脂に見えたので、絵を当てた。高さを 2 層（詰め物のうねりと粒）で作り、そこから法線と薄い濃淡を起こす。`loungeSofa.glb` には接線が無く法線が効かないため、接線付きの写しを `Assets/Models/generated/` に作って差し替えた。貼る細かさは UV の密度から決め、繰り返しが 0.6 メートルほどになる 0.055 に落ち着いた
- 艶の強さで 4 度振った。0.46 では平ら、0.64 では肘掛けが白飛び。0.58 で落ち着いた
- ドアを壁に沈めた。窓と同じで、枠が壁の前に立っていたのが原因。背面の壁を 3 枚に割り、枠より 0.01 メートル小さい穴を空けて、手前の面を壁と揃えた。穴が開いたぶん、扉に当たり判定を足してある

オーナーが手で動かした長椅子の位置と寸法には触れていない。壁に少し食い込んでいるのは意図とのこと。

### 締めの確認（段 4-b 完了）

部屋を作り替えた後、通しで壊れていないことを数値で確かめた。

- EditMode テスト 67 件すべて通過
- 調べる対象 8 点。意図した順（ジャック → 煙草 → 箱 → チップ → 端末 → ドア）で、それぞれを狙えばそれが選ばれる。灰皿は立って机に寄れば選べる
- 床の通行判定を 0.16 メートルの格子（37x37）で取り、胴体（半径 0.3、高さ 1.7）が通れる 840 区画のうち 838 が椅子から繋がっている。残り 2 区画は家具の裏の孤立した隙間
- どの対象にも 1.0〜1.25 メートルまで寄れる。選択の半径 2 メートルに十分収まる
- ドアと窓の開口は塞がっている。ドアは扉の当たり判定、窓は腰壁とガラスの当たり判定で止まる
- コンソールにエラー・警告なし

...
計画の実行後、段階全体を設計書と突き合わせるレビューを行い、実害のある 3 点を直した。以下は記録であり、上のタスクの記述は実行時のまま残してある。

- `80302b8` 暗転してから「続く」が出るまでの 1.5 秒、プレイヤーが歩けていた。`Stand` が立ち上がった後も毎フレーム移動可を書き直していたため、停止でも完了でも止められなかった。立ち終わったら書かないようにし、完了時に移動を止める。あわせて、停止中に届いた `PressInteract` を捨てずに持ち越すようにした（再生での確認を取りこぼさないため）
- `9830201` 導入の停止が `Time.time` の期限、カードの切り替えが `WaitForSeconds` の連なりで、別々の時計だった。停止が数フレーム先に切れ、最初の独白より前にジャックを抜ける隙間があった。カードごとに停止を掛け直し、最後のカードを消した直後に独白を出す形に変えた。これに伴い `IntroSeconds`（6.2 秒）が不要になり、タイトルの後にあった 1.7 秒の空白も消えた。この 1.7 秒は、カードの消え際の演出がある版から定数だけを写していたもの。演出は段階 4 で入れる
- `9830201` 導入が `OnEnable` から始まっていたため、部品を入れ直すと最初からやり直していた。`Start` に移し、場面が終わっていたら途中で止まるようにした。煙草の演出も二重に始まらないようにした
- `105a0c4` 文面のアセットが設計書どおりかを見るテストを足した（id の一覧、各項目の行数、印の有無、ドアの 2 つの文）。それまで確かめたのは 1 度きりの読み返しだけだった

...
計画の実行後、オーナーが実機で見て出た指摘を順に直した。以下は記録であり、上のタスクの記述は実行時のまま残してある。

- 見せ方を 3 度作り替えた。最初は上下から閉じる瞼、次にその縁をぼかして弧を付けた瞼、最後に動きを付けない切り替え。瞼はどう作っても安っぽく見えるという判断で、画面をそのまま黒くしてカードを出し、そのまま戻す形に落ち着いた。瞼のための画と、閉じ具合を受け取る仕組みは消した
- 弧の向きを一度誤った。中央を垂らす形にしたが、目は中央がいちばん開いて左右の端で閉じるので逆だった。直した後に方向ごと変わったため、この作り自体が残っていない
- タイトルのカードを 2 行から 1 行（`HALF AWARE／かたあはれ`）にした
- 3 枚目のふりがなが漢字の上端と重なっていたので、持ち上げる量を増やした。横位置は漢字の幅の 1 パーセントほどしかずれておらず、そのままにした
- 煙草を取った直後、体と視線が座り始めの向きへ 2.5 秒かけて滑らかに戻るようにした。そのあいだから吸い終わりまで見回しを受け付けない。`PlayerController` に `CanLook` と `Yaw` を足し、`SceneFlow` が `Player` を演出へ渡す
- 間の取り方は実機で何度か振って決めた。最終はカード 1.95 秒、部屋 1.8 秒、最初の間 0.8 秒、向き直し 2.5 秒で、煙草は全体 14.6 秒。4 つとも Inspector に出してあるので、再生しながら変えられる

作業の進め方で反省がある。凝った見た目を、方向を確かめる前に作り込んで 2 度差し戻された。次は簡単な形で先に見せ、方向が定まってから詰める。

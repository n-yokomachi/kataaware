...
Quaternius の Ultimate Modular Women（Suit）を一人称の体として入れた。**オーナーの判断で「後日差し替えを検討」となっており、これは仮置き**である。

今の状態:

- `Assets/Editor/BuildProtagonist.cs` のメニュー「HalfAware / Build the protagonist」で丸ごと組み直せる。手で組むと再生を抜けるたびに消えるため、手順をコードに残した
- 素体は Suit。黒のジャケット・白いシャツ・黒のズボンが最初から分かれている
- **素体に UV が無い**（全頂点が原点に潰れている）ため、モデルに絵を貼ることはできない。顔は前に薄い面（`FacePlate.asset`）をかぶせ、そちらに UV を持たせて `Textures/Face.png` を貼っている。元の目と眉は肌で塗り潰してある
- 顔の絵は PIL で生成。琥珀色の目、眉、鼻孔、口、そして左目の目尻寄りのほくろ
- ライダースの部品は自作。襟・襟返し 2 枚・ジッパーと引き手・腰のベルトと留め金・襟先の留め金。すべて胸の骨に付けて追従する
- 一人称では頭と顔の面を切ってある。鏡の反射と最後の邂逅で使うので消してはいない
- 体は `Player` の下、局所位置 (-0.014, 0, -0.075)。正面を向いたときは視界に入らず、うつむくと襟と肩が見える

差し替えるときに引き継ぐもの: 顔の絵と面、ライダースの部品、組み立て手順。素体だけを入れ替えれば残りは寸法の調整で足りる。

**まだ無いもの:** 座位の姿勢。今は `Idle_Neutral` の 1 枚を当てているだけで、場面 1 の座り始めに対応していない。同梱の 24 の動作に座りは無いので、骨を直接曲げるか、Quaternius の Universal Animation Library（座りを含む）を retarget する。

---

## 固有物は生成せず自分で組んだ（段 4-c の実際）

生成器を入れる話は、Hunyuan3D の territory 条項で止まり、代わりの調査まで進めた。その途中でオーナーから「このレベルなら自分で形を作って、画像は別で生成して貼れるのでは」という指摘があり、その通りだったので**全部こちらで組む方針に切り替えた**。

`Assets/Editor/ProcMesh.cs` が断面を並べて皮を張る道具で、箱を積むだけでは出せない先細りや丸みが作れる。絵は PIL で生成して貼る。

組み終わったもの:

| 物 | 作り |
|---|---|
| 灰皿 | 断面を回した旋盤。外壁を上り縁を越えて内壁を下るので、深さ 3 センチの本当の窪みがある。吸い殻は縁側に寄せてある |
| 煙草の箱 | 蓋を開けた箱、銀紙の帯、中の空洞。正面に『双鶴』の意匠を貼る。絵は PIL、字は Noto Sans JP（OFL）。下の帯は「喫煙は記憶の定着を妨げます／倫敦保健庁」 |
| メモリ架 | 16 の差込口を 2 段 8 列。うち 6 つにチップ。前面は一段落とした板、上下に縁、天面に放熱の筋、表示灯 2 つ |
| 椅子 | 裾の広い重い土台、先細りの支柱、前を少し上げた座面、24 度倒した背もたれと枕、肘掛け。右の肘掛けにジャックの受け口と表示灯、そこから垂れるケーブル。**当たり判定は持たせない**（座って始まる位置が椅子の中なので、塞ぐと動けなくなる） |
| 前腕 | 輪を 12 枚並べた腕と 4 枚の親指。今は体に置き換わり伏せてある |
| 顔 | 素体に UV が無いため、前にかぶせた面に貼る。琥珀色の目、眉、鼻孔、口、左目の目尻寄りのほくろ |
| ライダースの部品 | 襟・襟返し 2 枚・ジッパーと引き手・腰のベルトと留め金・襟先の留め金 |

生成器は結局要らなかった。残っているのは人体だけで、それは Quaternius の仮置き。

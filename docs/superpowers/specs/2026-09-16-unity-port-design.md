...
- 眩暈: 同じ Full Screen Pass に、ぼかしと二重像を数値で持たせる。同じ絵を少しずらして 2 枚重ね、ずれの向きをゆっくり回す。ぼかしは 2 枚が溶け合わない程度に留め、画面全体のゆっくりした漂いを少し足す。強さは 2 つの数値（ぼかし、二重像）で持ち、時間で減衰する
...
- 主観に手は描かない。前腕とジャックはカメラの子で、下を向いたときだけ表示（素材の設計書 5 節のまま）

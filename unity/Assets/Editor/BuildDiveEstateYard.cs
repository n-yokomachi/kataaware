...
namespace HalfAware.EditorTools
{
    ...
    public static partial class BuildDive
    {
        ...
        /// </summary>
        static void EstateGrounds(Transform place)
        {
            var y = new YardBanks();
            YardSurface(y);
            YardWall(y);
        ...
        static void YardRoad(YardBanks y)
        {
            ...
            y.Kerb.Box(new Vector3((west + east) * 0.5f, 0.06f, RoadFar + 0.1f), new Vector3(east - west, 0.12f, 0.2f));
            ...
            YardDoubleYellow(y, west, east, RoadFar - 0.25f - 0.28f);
        }
        ...
        const int NextBays = 9;
        /// <summary>隣の棟の間口。ここに窓が一つずつ並ぶ</summary>
        const float NextBay = 2.6f;
        /// <summary>隣の棟の階数</summary>
        const int NextFloors = 5;
        /// <summary>隣の棟の西の端</summary>
        const float NextWest = -13.1f;
        /// <summary>隣の棟の東の端。ここから東は中の棟の受け持ち</summary>
        const float NextEast = NextBay * (NextBays - 1) - 11.2f + 1.9f;             // 11.5
        /// <summary>隣の棟の高さ。階の高さから導くので、階数を直しても窓の列と屋上がずれない</summary>
        const float NextHigh = 1.6f + NextFloors * Floor;                            // 15.6
        ...
        {
            slabs.FaceZ(BlockFace, NextWest, NextEast, 0f, NextHigh, -1);

            // 階ごとの水平の帯。窓の列だけだと面が平らすぎて、棟の高さが出ない
            for (var j = 0; j < NextFloors; j++)
                panes.FaceZ(BlockFace - 0.12f, NextWest, NextEast, 1.22f + j * Floor, 1.38f + j * Floor, -1);
            for (var i = 0; i < NextBays; i++)
            {
                for (var j = 0; j < NextFloors; j++)
                {
                    var x = -11.2f + i * NextBay;
                    var y = 1.6f + j * Floor;
            ...
            panes.Box(new Vector3((NextWest + NextEast) * 0.5f, NextHigh + 0.25f, BlockFace - 0.18f),
                new Vector3(NextEast - NextWest, 0.50f, 0.36f));
        }
        ...
        static void EstateMidBlock(Bank mid, Bank panes, float x0, float x1, float z, int floors, bool westEnd)
        {
            ...
            // 窓の列。階ごとの帯と、間口ごとの四角
            var bays = Mathf.Max(2, Mathf.FloorToInt((x1 - x0 - 1.6f) / NextBay));

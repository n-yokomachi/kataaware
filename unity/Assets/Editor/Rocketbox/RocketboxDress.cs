...
{
    /// <summary>
    /// 片割れのワンピースを一から作る（既存の服の形は使わない）。参考画像の、白（<see cref="Cloth"/>）の綿か麻のロングワンピース。
    /// - 胴: 体の人（女大 18）の肌と服の面を型にして、数 mm 外へ出した面。襟ぐり（前は浅い V ネック、後ろは首に沿う小さな立ち上がり）・腰の切り替え・袖の付け根で、
    ///   三角を値の面で切る（三角の辺でぎざぎざにならない）。骨の付き方は型の頂点のまま
    /// - 袖: 上腕のまわりの筒を、中ほどで膨らませたパフ（<see cref="SleevePuff"/>）。袖口に帯。長さは上腕の <see cref="SleeveT1"/>。上腕の骨に付く
    /// - スカート: 腰の切り替えから、足首の少し上の裾（<see cref="HemY"/>）へたっぷり広がるフレアの筒。縦にゆるい波のひだを寄せる。
    ///   膝の少し下（<see cref="TierY"/>）で上の段と下の段に切り替え、裾はスカラップの縁

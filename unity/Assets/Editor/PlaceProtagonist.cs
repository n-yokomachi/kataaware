...
namespace HalfAware.EditorTools
{
    ...
    public static partial class PlaceProtagonist
    {
        ...
        static bool Room(GameObject player, Transform old, GameObject her, Animator an, float eyeLead, SceneFlow flow, StringBuilder note)
        {
            ...
            // 右の卓に置いたジャケットと、着る流れ（体のジャケットは脱いだ形で始める）
        ...
        /// 端末の画面に映る口元、右の卓に置いたジャケットとその調べる対象、ジャケットに場所を譲って切った明かり、前の置き場だった肘掛けのジャケット）の差は書くが、
        /// 思いがけない差には数えない
        /// </summary>
        public static string Diff(Dictionary<string, string> a, Dictionary<string, string> b, out int unexpected)
        {
            ...
            foreach (var k in keys)
            {
                ...
                var allowed = k == "Room/Chair/JackRest" || k == "Room/Chair/Cable" || k.EndsWith("/" + ReflectionName) || k.Contains("/" + ReflectionName + "/")
                    || k == "Room/" + FoldedName || k == "Room/" + LampName || k == "Room/Chair/" + OldDrapedName || k == "Interactables/" + JacketItemName;

...
{
    /// <summary>
    /// 遠景の書き割りを撮るためだけに組む、ロンドンの街並みの部品。公営住宅（<c>EstateTown</c>）と
    /// 公園（<c>ParkTown</c>）が分け合う。どちらも撮り終えたらその場で捨て、シーンにもアセットにも残さない。
    ///
    /// **撮り終えたら捨てるので、頂点の数は気にしない。** 細かく組むほど画が良くなる。
    /// 窓と煙突は等間隔にする。等間隔の繰り返しが、遠くでも長屋や高層棟だと読ませる。
    ///
    /// **置いてよい所は撮る目が決める**（<see cref="Town.Near"/>・<see cref="Town.Cap"/>）。
    /// 本物の地面の縁より内に置くと、板に貼ったときに足元が本物の地面に埋まる。
    /// 仰角の上限より高いと、板の上端で切れる
    /// </summary>
    public static partial class BuildDive
    {
        ...
        {
            var high = Mathf.Min(40f, t.Cap(t.Reach(foot) - radius) - 2f);
        ...
        /// <summary>
        /// 教会。石の身廊と、四角い塔の上の八角の尖塔。尖塔は向きを変えた細い箱を重ねて窄める。
        /// 中心から <paramref name="dist"/> m・方角 <paramref name="az"/> 度に、身廊の長手を <paramref name="yaw"/> 度へ向けて置く。
        /// 尖塔の先は仰角の上限か <paramref name="tallest"/> の低い方まで
        /// </summary>
        static void TownChurch(Town t, List<Vector3> keep, float dist, float az, float yaw, float tallest)
        {
            var at = t.At(dist, az);
            keep.Add(new Vector3(at.x, 26f, at.z));
            var rot = Quaternion.Euler(0f, yaw, 0f);
            System.Func<float, float, float, Vector3> p = (x, y, z) => at + rot * new Vector3(x, y, z);
            var cap = Mathf.Min(t.Cap(t.Reach(at) - 6f) - 1f, tallest);
            // 身廊
        ...
        /// <summary>
        /// シティの高いビル。中心から 360〜480 m、方角 <paramref name="azFrom"/> から <paramref name="azSpan"/> 度の幅に固める。ガラスの箱の塔と、先の尖った三角の塔、
        /// 卵形の塔、段々の塔。脇に塔の形のクレーンを四本（高さは <paramref name="craneMax"/> まで）。霞にほとんど溶けて、日を背にした輪郭だけが残る
        /// </summary>
        static void TownCity(Town t, int seed, float azFrom, float azSpan, float craneAz, float craneMax)
        {
            var rnd = new System.Random(seed);
            var placed = new List<Vector3>();
            for (var n = 0; n < 200 && placed.Count < 22; n++)
            {
                var az = azFrom + (float)rnd.NextDouble() * azSpan;
            ...
            {
                var dist = 380f + k * 22f;
                var at = t.At(dist, craneAz + k * 8.5f);
                var high = Mathf.Min(t.Cap(dist) - 3f, craneMax);
        ...
        /// 升目の向きは <paramref name="gridYaw"/> 度。他の物（高架・ガスタンク・教会・高層棟）の近くと、中心から <see cref="t.Near"/> より内は空ける
        /// </summary>
        static void TownRoofSea(Town t, List<Vector3> keep, int seed, float gridYaw)
        {
            var rnd = new System.Random(seed);
            ...
            {
                var z = j * pitch;
                var x = -t.Roofs - 20f + (float)rnd.NextDouble() * 20f;
                while (x < t.Roofs)
                {
                    var len = 20f + rnd.Next(0, 5) * 10f;
                    var mid = t.Centre + rot * new Vector3(x + len * 0.5f, 0f, z);
                    var from = t.Centre + rot * new Vector3(x, 0f, z);
                    var to = t.Centre + rot * new Vector3(x + len, 0f, z);
                    x += len + 7f + rnd.Next(0, 2) * 6f;
                    // 列のどこかが近すぎる、遠すぎる、他の物に掛かるなら置かない
                    if (Mathf.Min(t.Reach(from), Mathf.Min(t.Reach(to), t.Reach(mid))) < t.Near + 8f) continue;
                    if (TownOff(t.Centre, from, to) < t.Near + 8f) continue;
                    if (t.Reach(mid) > t.Roofs) continue;
                    if (t.Rail && TownOff(mid, t.RailFrom, t.RailTo) < len * 0.5f + 12f) continue;
                    var blocked = false;
                    ...
                    TownTerrace(t, mid, gridYaw, len, storeys, skin, t.Reach(mid) < t.Fine, rnd);
                }
            }
        }
        ...
        sealed class Town
        {
            ...
            /// <summary>書き割りの輪の中心。場所のローカル</summary>
            public Vector3 Centre;
            /// <summary>撮る目の高さ</summary>
            public float Eye;
            /// <summary>絵の中の街並みを置いてよい一番近い距離。本物の地面の縁の少し外</summary>
            public float Near;
            /// <summary>中心から見た仰角の上限。度。それより高いと板の上端で切れる</summary>
            public float Rise;
            /// <summary>長屋の屋根の海を敷く一番遠い距離</summary>
            public float Roofs;
            /// <summary>これより遠い長屋は窓と裏の張り出しを省く。霞が 6 割を超えて、細かさが画に出ない</summary>
            public float Fine;
            /// <summary>鉄道の高架。長屋が避ける線。無ければ Rail を立てない</summary>
            public bool Rail;
            public Vector3 RailFrom, RailTo;

            /// <summary>中心から距離 d の所に置ける物の高さの上限</summary>
            public float Cap(float d)
            {
                return Eye + d * Mathf.Tan(Rise * Mathf.Deg2Rad);
            }

            /// <summary>中心からの距離と角度（+z が 0、東回り）で場所のローカルの点</summary>
            public Vector3 At(float dist, float az)
            {
                return Centre + BackdropRing.Heading(az) * dist;
            }

            public float Reach(Vector3 p)
            {
                return new Vector2(p.x - Centre.x, p.z - Centre.z).magnitude;
            }

            /// <summary>中心から見た角度。0〜360</summary>
            public float Az(Vector3 p)
            {
                var a = Mathf.Atan2(p.x - Centre.x, p.z - Centre.z) * Mathf.Rad2Deg;

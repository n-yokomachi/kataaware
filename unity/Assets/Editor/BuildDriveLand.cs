    ...
    public static partial class BuildDrive
    {
        ...
        static void Clouds(Transform root)
        {
            ...
            for (var b = 0; b < Bands; b++)
            {
                ...
                // 三日月。**倫敦の市街に出す。**
                //
                // はじめは夜の高速へ置いていた。倫敦の黒雲（Nano）を抜けたところだから、
                // という理屈だったが、あちらは土砂降りの中を走る帯で、
                // 輪郭のはっきりした三日月とは噛み合わなかった。オーナーの指示でこちらへ移す。
                //
                // ナノマシンの黒雲との辻褄は、切れ間として通る。原作の黒雲は空を塗り潰す蓋ではなく
                // クラッカーが撒いたものが溜まっているもので、ビルの谷の上だけ裂けていて構わない。
                // **方位はほとんど正面に置く。** 市街は道の両側にビルが立っているので、
                // 横へ振ると近いビルの中に入って見えない。谷がまっすぐ抜けている正面なら空が残る。
                //
                // 最後の帯（朝靄の未舗装路）には出せない。塗り潰しが靄の白（0.820）なので、
        ...
        /// 月の方位。度。**0 で道の消失点に重なる。**
        ///
        /// 道は +z に平行なので、その消失点はカメラの正面そのもの。横へ振ると
        /// 市街ではビルの谷から外れて近いビルに隠れる。真ん中に置くのが
        /// 見えるうえに収まりもよい
        /// </summary>
        const float MoonSwing = 0f;
        ...
        /// <summary>
        /// 月の傾き。度。**膨らみを下へ、真下からは少し外す。**
        ///
        /// <see cref="Crescent"/> は欠けさせる円を +x へずらして作るので、傾き 0 では
        /// 膨らみが左（-x）を向く。そこから左回りに 90 度で真下になるが、
        /// ぴたり真下だと器を伏せたように左右対称で、絵として硬い。
        /// 112 度で膨らみが右下（0.37, -0.93）、角（つの）が左上を向く。
        /// 日の沈んだ西を向く三日月の傾きでもある
        /// </summary>
        const float MoonTilt = 112f;
        ...
        /// <summary>
        /// 道の中心から木までの距離。m。左側に立てる。
        ///
        /// **霧が距離の上限を決める。** 帯 4 の霧は一乗の 0.0080 で、190 m では 78% 霞む。
        /// そこでは何を置いても靄の色（0.82）の 8 割が乗るので、どんなに暗い色を与えても
        /// 空との差が画面で 2 しか残らず、木ではなく雲の切れ端に見えた。
        /// 130〜140 m まで寄せると 65% で、空 0.92 に対して木が 0.79 に沈む
        /// </summary>
        const float LoneTreeAt = 72f;

        /// <summary>
        /// 木の足の高さ。m。道から 72 m のところの丘の高さ（<see cref="Land"/> でおよそ 8.1 m）。
        ///
        /// **足を丘の裏へ隠し切ってはいけない。** 4.6 m まで下げたときは幹が丸ごと稜線に
        /// 隠れて、樹冠だけが地平に浮いた塊になった。木に見えるのは樹冠の形ではなく、
        /// 幹が地面から立ち上がっているところ
        /// </summary>
        const float LoneTreeFoot = 8.0f;

        /// <summary>
        /// 走った距離に対して木が動く割合。
        ///
        /// 1/20 なら、この帯を 880 m 走るあいだに 44 m しか動かない。
        /// 道の脇 10 m の木が 1 秒で通り過ぎるのに対して、こちらは 80 秒かけて
        /// 斜め前 26 度から 34 度へ回り、目からの距離は 164 m から 126 m にしか縮まらない。
        /// それが「遠くにある」ということ。
        ///
        /// 1/12 では 80 秒で 73 m 動いて、距離が 139 m から 96 m まで詰まった。
        /// 木がはっきり大きくなっていくので、遠景ではなく近づいてくる物に見える
        /// </summary>
        const float LoneTreeCreep = 1f / 20f;

        /// <summary>
        /// 木が出てくる z。m。
        ///
        /// **近づけすぎない。** 120 から始めたときは、80 秒走ったところで 104 m まで
        /// 詰まって、樹冠の天が仰角 20 度まで上がり、窓の縁に切られて灰色の板になった。
        /// 148 から始めれば、帯を走り切っても 126 m より近づかない
        /// </summary>
        const float LoneTreeFrom = 148f;

        /// <summary>木を戻す z。m。背中のずっと後ろ。画面に残っているところで戻すと跳ぶのが見える</summary>
        const float LoneTreeUntil = -212f;

        /// <summary>
        /// 帯 2 の遠景に一本だけ立つ木。
        ///
        /// 原作にある物ではないが、朝靄の畑がどこまでも同じ調子で広がるので、
        /// 目を留める先がどこにも無かった。遠くに一本だけ大きな木が立つと、
        /// 畑の広さがその木を物差しにして読めるようになる。
        ///
        /// **沿道（<see cref="Roadsides"/>）には置けない。** あちらは 180 m の環なので、
        /// 16 秒に一度おなじ木が戻ってくる。空の物として置いて、走った距離の 1/12 だけ
        /// 手前へ動かす（<see cref="FarTree"/>）。動きだけを遠くのものにして、
        /// 置く場所は霧の届く 100〜200 m に留める。帯 4 の霧（0.0080）では
        /// 250 m より先は何も見えないので、本当に遠くへは置けない。
        ///
        /// **左に立てる。** 日は後ろ寄りの右（方位 298 度）から差すので、こちらから見える面は
        /// 左に立てたときだけ日を受ける。右に置くと、通り過ぎるあいだ陰の側しか見えない。
        ///
        /// 空の入れ物に入れておけば、帯の出し分け（DriveDirector.ShowSky）に連れて
        /// 出入りする。区切りの数を数える沿道と違って、いくつ入れても環は狂わない
        /// </summary>
        static void LoneTree(Transform band)
        {
            var mesh = Shape("LoneTree", 0.25f, b =>
            {
                var rnd = new System.Random(8831);
                // 幹。**短く太く。** 樹冠の裾（4 m あたり）のすぐ上までしか要らない。
                //
                // 細い幹の上に樹冠を高く載せると、地平では茸か給水塔に見える。
                // 遠くの一本木で目に入るのは樹冠の塊で、幹は足元に少し覗くだけ。
                // 樹冠の中まで通しておくのは、隙間が空くと樹冠が宙に浮くため
                b.Box(new Vector3(0f, 3.5f, 0f), new Vector3(2.6f, 7.0f, 2.6f));
                // 樹冠。**丸い塊を重ねて作る。** 一つの箱では、靄に沈んでも箱のままに見える。
                // 大きさと位置をばらしながら 10 個重ねると、縁が凸凹して樹冠の丸みが出る。
                //
                // **天へ向かって細らせる。** 上まで同じ幅で積んだら、幹の上に板が載った
                // 給水塔になった。いちばん広いところを裾寄り（下から三分の一）に置いて、
                // そこから上へ窄めると、枝を張った広葉樹の形になる。
                //
                // **細らせるのは形のためだけではない。** 地平の空は靄の白（仰角 5 度まで）で、
                // そこから上は空の板が青を混ぜはじめる（15 度で青のまま）。霧が 6 割掛かる
                // 距離では木の色は 0.75 より暗くならないので、青い空を背にした部分は
                // 木の方が明るくなって形が読めない。天を細らせておけば、そこは
                // 「靄へ溶けていく穂先」として収まる。太いまま上げると灰色の長方形が浮く。
                // 背は幅の 0.8 倍。裾の高さも塊ごとに振って、下端が水平線にならないようにする
                var lobes = new[]
                {
                    new Vector4(0.0f, 10.5f, 0.0f, 13.0f),
                    new Vector4(-6.6f, 9.6f, 0.8f, 9.6f),
                    new Vector4(6.8f, 9.2f, -0.9f, 10.0f),
                    new Vector4(-3.8f, 13.8f, -1.2f, 8.6f),
                    new Vector4(4.0f, 14.2f, 1.1f, 8.2f),
                    new Vector4(-1.2f, 16.6f, 0.2f, 5.8f),
                    new Vector4(2.2f, 16.0f, -0.8f, 5.0f),
                    new Vector4(0.4f, 18.6f, 0.0f, 3.6f),
                    new Vector4(-3.0f, 6.6f, 0.6f, 7.0f),
                    new Vector4(3.2f, 6.2f, -0.4f, 6.4f),
                };
                for (var i = 0; i < lobes.Length; i++)
                {
                    var l = lobes[i];
                    var wide = l.w;
                    b.Box(new Vector3(l.x, l.y, l.z), new Vector3(wide, wide * 0.72f, wide * 0.88f),
                        Quaternion.Euler(0f, (float)rnd.NextDouble() * 90f, 0f));
                }
            });
            // 木の色。**地の素材（Tree 0.045）ではこの距離では出ない。**
            //
            // 靄が 65% 掛かるので、画面に出る色は「靄の色 × 0.65 ＋ 木の色 × 0.35」になる。
            // 暗いだけの色を置くと日の当たった側と陰の側の差まで 0.35 倍に潰れて、
            // 片側が明るいどころか一様な灰の塊になる。実際 0.15 で撮ったら、
            // 明暗の差が sRGB で 0.02 しか残らず、雲の切れ端にしか見えなかった。
            //
            // 遠いものは空気に洗われて明るくなる。0.22 まで上げると、日の当たった側が
            // 0.83、陰の側が 0.78 で、空（0.92）よりはっきり暗いまま片側だけが明るい
            var tree = Piece(band, "LoneTree", mesh, CityMat("LoneTree", new Color(0.228f, 0.218f, 0.132f), 0.05f));
            tree.localPosition = new Vector3(Lane(-LoneTreeAt), LoneTreeFoot, LoneTreeFrom);
            var far = tree.gameObject.AddComponent<FarTree>();
            var so = new SerializedObject(far);
            so.FindProperty("parallax").floatValue = LoneTreeCreep;
            so.FindProperty("from").floatValue = LoneTreeFrom;
            so.FindProperty("until").floatValue = LoneTreeUntil;

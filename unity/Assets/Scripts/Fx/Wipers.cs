...
namespace HalfAware
{
    ...
    public sealed class Wipers : MonoBehaviour
    {
        ...
        /// <summary>
        /// 振りの中心。度。真上が 0、運転席の側（+x）へ回る向きが正。
        ///
        /// **伏せた角（中心 - 振り幅）が -84 度から動かないように、振り幅と対で動かす。**
        /// 伏せた姿勢はボンネットの陰に隠れていることを実際に撮って確かめてあり
        /// （組み立ての見直し 14 も、羽根を伏せたまま道の見える縁との隙間を測っている）、
        /// 片方だけ動かすとその隙間が黙って詰まる
        /// </summary>
        public const float Centre = -24f;
        /// <summary>
        /// 振り幅。度。片側の値。
        ///
        /// 伏せた側（中心 - 振り幅 = -84 度）ではほとんど寝ていて、ボンネットの陰に隠れる。
        /// 起こした側（+36 度）で運転席の正面を越えて助手席の側まで払う。
        /// 運転席から風防を真っ直ぐ見た点は軸から 4.5 度のところにあるので、
        /// ここを詰めると正面が拭われないまま残る。
        ///
        /// **49 度から 60 度へ広げた。** 払う弧が 98 度から 120 度になり、
        /// 一往復で拭う面積が 1.2 倍になる。土砂降りのガラスでは、羽根が何度も
        /// 同じ狭い扇を往復しているより、間口を大きく薙ぐ方が拭っていることが読める。
        ///
        /// **これ以上は広げない。** 起こした側を +36 度より倒すと、羽根が立ったまま
        /// 折り返すのをやめて視界の真ん中へ寝てくる。伏せた側も -84 度より下げると、
        /// 羽根の先がガラスの下の縁（軸の 20 mm 下）を割って水切り板の中へ入る
        /// </summary>
        public const float Sweep = 60f;
        /// <summary>
        /// 一往復にかける秒数。
        ///
        ...
        /// 拭っているのかどうか画面から読めない。
        ///
        /// **土砂降りなので半周より短く取る。** 0.8 だった頃は真ん中が 6 割までしか
        /// 戻らず、いちばん目に入る正面がいつも拭われかけの薄い水で埋まっていて、
        /// 雨が降っているように見えなかった。0.45 なら半周（0.47 秒）でちょうど
        /// 溜まりきるので、羽根が抜けた跡へ水が戻りきるところまで見える。
        /// 拭った瞬間は 0 のままなので、拭っていることは変わらず読める
        /// </summary>
        public const float Dry = 0.45f;
